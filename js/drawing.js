...
export function drawBackgroundAndPath(ctx) {
    ...
    // Sprawdzenie, czy ścieżka nie wychodzi poza obszar gry (opcjonalne, bardziej dla debugowania)
    // let pathIsSafe = true;
    // for(const node of state.currentPath){
    //     if(node.y > C.MAX_GAME_ROW) {
    //         pathIsSafe = false;
    //         break;
    //     }
    // }
...
export function drawTheaterBase(ctx) {
    ...
    if (baseImg && !baseImg.error) {
        ...
        const baseNode = state.currentPath[state.currentPath.length -1];
        const baseY = Math.min(baseNode.y, C.MAX_GAME_ROW -1);
        ctx.fillRect( (C.COLS - 1.5) * C.TILE_SIZE, (baseY - 0.5) * C.TILE_SIZE, fallbackSize, fallbackSize);
    }
}
...
export function drawWaveIntro(ctx) {
    ...
    if (state.waveIntroTimer <= 0) {
        // Funkcja startNextWaveActual będzie importowana i wywoływana w main.js lub gameLogic.js
        // Tutaj tylko dekrementujemy timer, logikę startu fali przeniesiemy
    }
}
...
export function drawUI(ctx) {
    ...
    // Przycisk Fullscreen
    const fsRegion = state.uiRegions.fullscreenButton;
    if (fsRegion) { // Dodano sprawdzenie, czy fsRegion jest zdefiniowany
        const fsText = document.fullscreenElement ? "FS-" : "FS+";
        ctx.fillStyle = "#607D8B";
        ctx.fillRect(fsRegion.x, fsRegion.y, fsRegion.width, fsRegion.height);
        ctx.strokeStyle = "#37474F";
        ctx.lineWidth = 2;
        ctx.strokeRect(fsRegion.x, fsRegion.y, fsRegion.width, fsRegion.height);
        ctx.lineWidth = 1;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        drawTextWithOutline(ctx, fsText, fsRegion.x + fsRegion.width / 2, fsRegion.y + fsRegion.height / 2, C.UI_FONT_SMALL, "#FFFFFF", "rgba(0,0,0,0.7)");
    }
...
export function drawGameOverScreen(ctx) {
    ...
    ctx.fillStyle = state.currentMessage.includes("GRATULACJE") ? "lime" : "red";
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ...
    ctx.font = "20px Arial";
    ctx.fillStyle = "white";
    ctx.fillText("Odśwież stronę (F5), aby zagrać ponownie.", ctx.canvas.width/2, ctx.canvas.height/2 + 30);
    ctx.textBaseline = "alphabetic";
}